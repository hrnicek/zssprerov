...
class PrintMenuController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->has('jidelna')) {
            ...
            // Find the establishment details
            $selectedEstablishment = $establishments->firstWhere('code', $establishmentCode);
            ...
            try {
                ...
                Log::error('Failed to fetch menu data for print: '.$e->getMessage());
            }
        }

        return inertia('menus/Print', [
            'establishments' => $establishments,
            'menuData' => $this->formatMenuDataForTable($menuData),
    ...
    /**
     * Format menu data specifically for table display in print view
     */
    private function formatMenuDataForTable(array $menuData): array
    {
        $formattedData = [];
        
        foreach ($menuData as $day) {
            $dayFormatted = [
                'day' => $day['name'],
                'date' => $day['date'],
            ...
            // Group meals by type for better table structure
            $mealsByType = [];
            ...
            $dayFormatted['meals'] = $mealsByType;
            $formattedData[] = $dayFormatted;
        }
    ...
    private function parseMenuXml(string $xmlContent, array $selectedDates = []): array
    {
        ...
        try {
            ...
            Log::error('Failed to parse menu XML for print: '.$e->getMessage());
        }

        return $menuData;
    }

    private function generateThreeWeekDates(string $startDate = null): array
    {
        $dates = [];
        $startDate = $startDate ? new \DateTime($startDate) : new \DateTime();

        try {
            // Generate 21 days (3 weeks) starting from the given date
            for ($i = 0; $i < 21; $i++) {
            ...
            Log::error('Failed to generate three week dates for print: '.$e->getMessage());
    ...
    {
        // This method can be implemented later for PDF generation
        // For now, redirect to the print view
        return redirect()->route('print.menus', $request->all());
    }
}